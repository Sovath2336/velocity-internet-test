...
const App: React.FC = () => {
  ...
  // Refs for smooth animation and throttled state updates
  ...
  // Smooth gauge value updater using requestAnimationFrame
    ...
    const stream = async () => {
      ...
      try {
        ...
        while (true) {
          ...
          // Batch progress and gauge updates for smoothness
  ...
  const renderFooterContent = () => {
    switch(footerSection) {
        ...
        return (
          <div className="w-full max-w-4xl glass rounded-[2.5rem] p-10 animate-in fade-in slide-in-from-bottom-6 duration-500">
            ...
            <div className="space-y-4 text-white/70 text-sm leading-relaxed">
              <p>At Velocity Core, we prioritize your digital sovereignty. Unlike traditional speed tests, we do not track your specific IP address for advertising profiles.</p>
              <p><strong>Data Collection:</strong> We collect non-identifiable telemetry including connection speed, ISP name, and generalized geographic region (City/Country) only to improve our global edge network performance.</p>
              <p><strong>No Cookies:</strong> We operate a cookie-free environment for testing. Your history is stored locally in your browser's <code>LocalStorage</code> and never touches our servers.</p>
            </div>
        ...
        return (
          <div className="w-full max-w-4xl glass rounded-[2.5rem] p-10 animate-in fade-in slide-in-from-bottom-6 duration-500">
            ...
            <div className="space-y-6 text-white/70 text-sm leading-relaxed">
              <section>
                <h3 className="text-white font-bold mb-2">1. Socket Saturation</h3>
                <p>Velocity opens 4 concurrent HTTP/3 or HTTPS streams to eliminate the effects of TCP slow-start and single-connection bottlenecks. This measures the total pipe capacity of your ISP.</p>
              </section>
              <section>
                <h3 className="text-white font-bold mb-2">2. Anycast Routing</h3>
                <p>We test against Anycast IP addresses. Your requests are automatically routed to the closest physical server node, ensuring the "last mile" of your connection is the measured bottleneck, not server-side distance.</p>
              </section>
              <section>
                <h3 className="text-white font-bold mb-2">3. Jitter & Latency</h3>
                <p>We perform 6 rapid-fire ICMP-equivalent requests. We discard the highest and lowest results to calculate a stable median ping and precise jitter (variance in arrival time).</p>
              </section>
            </div>
            <button onClick={() => setFooterSection(null)} className="mt-10 w-full py-4 bg-white/5 hover:bg-white/10 rounded-2xl text-xs font-bold uppercase tracking-widest transition-all text-white">Return to Engine</button>
          </div>
      ...
      case 'enterprise':
        return (
          <div className="w-full max-w-4xl glass rounded-[2.5rem] p-10 animate-in fade-in slide-in-from-bottom-6 duration-500">
            <h2 className="text-2xl font-bold text-white mb-6 uppercase tracking-widest border-b border-white/10 pb-4">Enterprise Solutions</h2>
            <div className="space-y-6 text-white/70 text-sm leading-relaxed">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-6 bg-white/5 rounded-2xl border border-white/5">
                  <h4 className="text-blue-400 font-bold mb-2">Custom Nodes</h4>
                  <p className="text-xs">Deploy Velocity testing endpoints inside your private corporate network or VPC for internal SLA monitoring.</p>
                </div>
                <div className="p-6 bg-white/5 rounded-2xl border border-white/5">
                  <h4 className="text-purple-400 font-bold mb-2">API Access</h4>
                  <p className="text-xs">Programmatic access to raw throughput data for automated network troubleshooting and load balancing.</p>
                </div>
              </div>
              <p className="text-center italic mt-4 opacity-50">Contact the Velocity Core Infrastructure team for custom integration requirements.</p>
            </div>
            <button onClick={() => setFooterSection(null)} className="mt-10 w-full py-4 bg-white/5 hover:bg-white/10 rounded-2xl text-xs font-bold uppercase tracking-widest transition-all text-white">Return to Engine</button>
          </div>
        );
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-4 md:p-8">
      {copySuccess && (
        <div className="fixed top-8 left-1/2 -translate-x-1/2 z-50 glass px-6 py-3 rounded-2xl border-blue-500/50 border animate-in slide-in-from-top-4">
          <span className="text-xs font-bold tracking-widest text-blue-400">RESULT LINK COPIED TO CLIPBOARD</span>
        </div>
      )}
              ...
              <div className="flex flex-col items-center justify-center gap-10 order-1 lg:order-2">
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  {phase === TestPhase.COMPLETE && (
                    <button 
                      onClick={shareResult}
                      className="flex items-center gap-2 text-[10px] font-bold opacity-40 hover:opacity-100 uppercase tracking-widest transition-opacity py-2"
                    >
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-3 gap-8 text-sm opacity-60 mb-20 mt-10">
        <section className="glass p-8 rounded-[2rem] border border-white/5">
          <h3 className="font-black text-white mb-4 uppercase tracking-widest text-xs">Accuracy First</h3>
          <p className="leading-relaxed text-white/80">Velocity uses concurrent socket saturation to measure true throughput. We discard initial latency spikes to ensure your reported speed is what your connection can actually sustain.</p>
        </section>
        <section className="glass p-8 rounded-[2rem] border border-white/5">
          <h3 className="font-black text-white mb-4 uppercase tracking-widest text-xs">Edge Technology</h3>
          <p className="leading-relaxed text-white/80">Tests are performed against global anycast networks. This ensures the bottleneck measured is your actual ISP connection, not distant server congestion.</p>
        </section>
        <section className="glass p-8 rounded-[2rem] border border-white/5">
          <h3 className="font-black text-white mb-4 uppercase tracking-widest text-xs">Real-Time Data</h3>
          <p className="leading-relaxed text-white/80">Watch your connection's stability in real-time. Our charts visualize throughput variations, helping you identify packet loss or intermittent throttling.</p>
        </section>
      </div>
      ...
      <footer className="w-full max-w-6xl py-12 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-10 opacity-40 text-[10px] font-bold uppercase tracking-widest">
        <div className="flex items-center gap-6">
          <span className="text-white">&copy; 2025 VELOCITY CORE</span>
          <span className="text-blue-500">Tier 1 Analytics</span>
        </div>
